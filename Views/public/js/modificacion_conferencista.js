...
function validateForm(){ 
    ...
    var cedulaErr = nombreErr = celular1Err= celular2Err = correoErr = perfilErr= linkedinErr = paisErr= true;


// profe en esta parte no puedo hacer que en el campo Nombre me incluya acentos
    // Validate nombre
   if(nombre == "") {
        printError("nombreErr", "Por favor ingrese su nombre");
    } else {
        var regex = /^[ A-Za-zäÄëËïÏöÖüÜáéíóúáéíóúÁÉÍÓÚÂÊÎÔÛâêîôûàèìòùÀÈÌÒÙ.-]{8,70}$/;
        //var regex = /^[a-zA-áéíóúñüàè]{5,50}$/;
        if(regex.test(nombre) === false) {
            printError("nombreErr", "Por favor ingrese su Nombre Completo valido y con Max 70 letras");
        } else {
    ...
    if(cedula == "") {
        ...
        if(regex.test(cedula) === false) {
            printError("cedulaErr", "Por favor ingrese un Numero de identificación valido de 10 digitos");
        } else{
...
if(celular1 == "") {
        ...
        if(regex.test(celular1) === false) {
            printError("celular1Err", "Por favor ingrese un Numero de Telefono valido de 10 digitos");
        } else{
    ...
    if(celular2 == "") {
        ...
        if(regex.test(celular2) === false) {
            printError("celular2Err", "Por favor ingrese un Numero de telefonico valido de 10 digitos");
            celular2Err = true;
        } else{
...
if(correo == "") {
        ...
        var regex = /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,15}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,20}[a-zA-Z0-9])?)*$/;
    ...
    if(perfil == "") {
        ...
        var regex = /^[ A-Za-zäÄëËïÏöÖüÜáéíóúáéíóúÁÉÍÓÚÂÊÎÔÛâêîôûàèìòùÀÈÌÒÙ.,:;!#$%&'*+/=?^_`{|}~1234567890-]{10,400}$/;
        //var regex = /^[ A-Za-zäÄëËïÏöÖüÜáéíóúáéíóúÁÉÍÓÚÂÊÎÔÛâêîôûàèìòùÀÈÌÒÙ.-]{10,110}$/;
        if(regex.test(transformed) === false) {
            printError("perfilErr", "Por favor ingrese una descripción valida y con Max 400 letras");
        } else {
    ...
    if(pais == "") {
        ...
        var regex = /^[ A-Za-zäÄëËïÏöÖüÜáéíóúáéíóúÁÉÍÓÚÂÊÎÔÛâêîôûàèìòùÀÈÌÒÙ.-]{1,50}$/;
        if(regex.test(pais) === false) {
            printError("paisErr", "Por favor ingrese su Nombre Completo valido y con Max 50 letras");
        } else {
            printError("paisErr", "");
            paisErr = false;
        }
    ...
    if ((nombreErr || celular1Err || celular2Err || correoErr || perfilErr || linkedinErr || paisErr || cedulaErr) == true) {
        return false;
    } else {
