        ...
        $(document).ready(function(){
           // $('#idinstitucion').select2();
            ...
            if (tipoevento==5 || tipoevento==1) {
                if ((dd==dia_evento_inicio) && (mm==mes_evento_inicio) && (yyyy==año_evento_inicio)) {
                    if ((hora_actual>=horas_evento_inicio) && (hora_actual<=horas_evento_fin)) {
                        if ((hora_actual>=horas_evento_inicio) &&(minuto_actual>=minutos_evento_inicio)) {
                        ...
                        alert('Esta Reunión todavía no empieza o ya se terminó. Sólo admite inscripción el mismo día desde una hora antes, hasta la hora de finalización');
                        document.contactForm.submit.disabled=true;
                    }
                }
                else{
                    alert('Hoy no es la Reunión. Sólo admite inscripción el mismo día desde una hora antes, hasta la hora de finalización');
                    document.contactForm.submit.disabled=true;
                }
