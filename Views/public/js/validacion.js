...
function validateForm(){
    ...
    if(name == "") {
        ...
        var regex = /^[ A-Za-zäÄëËïÏöÖüÜáéíóúáéíóúÁÉÍÓÚÂÊÎÔÛâêîôûàèìòùÀÈÌÒÙ1234567890ñÑ.,-]{8,50}$/;
        //[a-zA-Z\s]
         
        if(regex.test(name) === false) {
            printError("nameErr", "Por favor ingrese su Nombre Completo valido y con Max 50 letras");
        } else {
            printError("nameErr", "");
            nameErr = false;
        }
    ...
    if(cual == "") {
        ...
        cualErr = false;
         ...
         if(regex.test(cual) === false) {
            printError("cualErr", "Por favor ingrese una Entidad o Institución valida");
            cualErr = true;
        } else{
            printError("cualErr", "");
            cualErr = false;
    ...
    if(idasistente == "") {
        printError("idasistenteErr", "Por favor ingrese un Numero de identificación");
    } else {
        var regex = /^[1-9]\d{4,9}$/;
        if(regex.test(idasistente) === false) {
            printError("idasistenteErr", "Por favor ingrese un Numero de identificación valido de 10 digitos");
        } else{
    ...
    if(mobile == "") {
        ...
        if(regex.test(mobile) === false) {
            printError("mobileErr", "por favor ingrese un numero telefonico de 10 numeros");
        } else{
    ...
    if ((nameErr || idasistenteErr || mobileErr || emailErr || genderErr || cualErr) == true) {
        return false;
    } else {
